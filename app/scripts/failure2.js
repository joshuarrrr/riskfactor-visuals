...
d3.chart("MarginChart").extend("FailureChart", {
    ...
    initialize: function() {
      ...
      chart.layers.areaBase3 = chart.base.select("g").append("g")
        .classed("area3", true);

      chart.layers.lineBase3 = chart.base.select("g").append("g")
        .classed("line3", true);
      ...
      // add area layer 3
      this.layer("spentarea", chart.layers.areaBase3, {
        modes : ["web", "tablet"],
        dataBind: function(data) {
        ...
        events: {
          ...
          "enter:transition" : function() {
            ...
            function pathTween() {
              var interpolate = d3.scale.quantile()
                .domain([0,1])
                .range(d3.range(1, chart.data.length + 1));
              return function(t) {
                return area(chart.data.slice(0, interpolate(t)));
              };
            }
      ...
      this.layer("initline", chart.layers.initLine, {
        ...
        events: {
          "merge" : function() {
            var chart = this.chart();
            var selection = this;
            var lineY = chart.yScale(chart.data[0].estMillions);
            
            var line = {
              "x1": function(d) { return chart.xScale(chart.data[0].dateObject); },
              "y1": function(d) { return chart.yScale(chart.data[0].estMillions); },
              "x2": function(d) { return chart.xScale(chart.data[chart.chart.data.length - 1].schedObject); },
              "y2": function(d) { return chart.yScale(chart.data[0].estMillions); },
            };
        ...
        "merge:transition" : function() {
          ...
          var line = {
            "x1": function(d) { return chart.xScale(chart.data[0].dateObject); },
            "y1": function(d) { return chart.yScale(chart.data[0].estMillions); },
            "x2": function(d) { return chart.xScale(chart.data[chart.data.length - 1].schedObject); },
            "y2": function(d) { return chart.yScale(chart.data[0].estMillions); },
          };
...
d3.csv("sampleproject.csv", function (dataOrig) {
  "use strict";
  ...
  var dataToPlot = dataOrig.slice(0);
