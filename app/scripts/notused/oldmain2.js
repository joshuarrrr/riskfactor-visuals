(function(){
  ...
  var purple = "#9B32C3";
  var yellow = "#FFD502";
  var darkgreen = "#478331";
  var darkblue = "#16769D";

  var color = d3.scale.ordinal()
    .range([orange, blue, yellow, red, purple, green, darkblue, "#fff"]);

  var format = d3.time.format("%B %d, 20%y");
  var formatString = d3.time.format("%m/%d/%y");
  ...
  data.forEach(function(d) {
    ...
    d.records = (parseInt(d.records) > 0) ? parseInt(d.records) : 0;
  });
  var sorted = data.sort(function(a,b){ return b.records - a.records; });
  var topten = [];
  var toptenKeys = [];
  ...
  var sums = d3.nest()
    ...
    .rollup(function(values) { return d3.sum(values, function(d) {return parseInt(d.records); }) })
    .map(data);
  ...
  var barScale = d3.scale.linear()
    .domain(d3.extent(topten, function(d) { return d.records; }))
    .range([10,450]);
  ...
  // org names
  var sectionLabel = svg.append("g")
  .selectAll(".section-labels")
    .data(topten)
  .enter().append("text")
    .attr("class","section-labels")
    .attr("x",0)
    .attr("y",function(d) { return barY(d.org)+15; })
    .text(function(d,i) { return i+1+". "+d.org; })
  ...
  .enter().append("rect")
    .attr("class","bars")
    .attr("x",375)
    .attr("y",function(d) { return barY(d.org) })
    .attr("width",function(d) { return barScale(d.records) })
    .attr("height",20)
    .style("fill",function(d){
      if (d.featured > 0) {
        return "#fff";
  ...
  .enter().append("text")
    ...
    .attr("x",function(d) { return barScale(d.records)+380 })
    .attr("y",function(d) { return barY(d.org)+17 })
    .text(function(d) { return (d.records/1000000).toFixed(1)+"M"; })
    .style("fill",function(d){
      if (d.featured > 0) {
        return "#fff";
      } else {
  ...
  .enter().append("line")
    ...
    .attr("y1",function(d) { return barY(d.org)+30 })
    .attr("y2",function(d) { return barY(d.org)+30 });
  ...
  // TIMELINE
  //
  var tt = d3.select("#chart2").append("div")   
    .attr("class", "tooltip")               
    .style("opacity", 0);
  var industries = [
    "Retail & Merchant",
  ...
  var margin = {top: 30, right: 50, bottom: 0, left: 0},
    width = 910 - margin.left - margin.right,
  ...
  // text
  svg.append("g")
  .selectAll(".industry-labels")
    .data(industries)
  .enter().append("text")
    .attr("class","industry-labels")
    .attr("x",210)
  ...
  .enter().append("text")
    ...
    .attr("x",function(d) { return x(formatString.parse("01/01/"+d))+220 })
    .attr("y",height-70)
  ...
  .enter().append("line")
    ...
    .attr("x1",function(d) { return x(formatString.parse("01/01/"+d))+220 })
    .attr("x2",function(d) { return x(formatString.parse("01/01/"+d))+220 })
    .attr("y1",height-95)
    .attr("y2",height-85);

  // lines
  svg.append("g")
  .selectAll(".industry-lines")
    .data(industries)
  .enter().append("line")
    .attr("class","industry-lines")
    .attr("x1",220)
    .attr("x2",width)
  ...
  // circles
  svg.append("g")
  .selectAll(".breach-dots")
    .data(data.sort(function(a,b) {
      return b.records - a.records;
    }))
  .enter().append("circle")
    .attr("class","breach-dots")
    .attr("cx",function(d) {return x(d.formattedDate)+220; })
    .attr("cy",function(d) { return y(d.industry) })
    .attr("r", function(d) { return radius(d.records); })
  ...
  // rollover
  d3.selectAll(".breach-dots")
    .on("mouseover",function(d) {
      var selected = d3.select(".button-active").attr("id");
      selected = (selected == "type") ? "source" : "industry";
      d3.select(".tooltip")
        .style("left",x(d.formattedDate)+220-122+"px")
        .style("top",y(d[selected])-80+"px")
        .style("opacity",1)
        .html(d.org+"<br><span style='font-weight:bold'>"+numberWithCommas(d.records)+" records breached</span><br>"+d.date);
      d3.select(this)
        .style("opacity",1)
        .style("stroke","#ccc");
    })
  ...
  d3.selectAll(".bttn").on("click",function() {
    var id = this.id;
    $("#"+id).addClass("button-active").siblings(".bttn").removeClass("button-active");
    if (id == "industry") {
      y.domain(industries);
      d3.selectAll(".breach-dots")
